...
@Slf4j
@Service
@ThirdPartySvcAnno(provider = Constant.PROVIDER_CODE, service = Constant.REMOTE_VEH_CONTROL_AIR_TEMP_API)
public class SmartHomeRemoteVehControlAirTempServiceImpl implements SmartHomeRemoteVehControlIService, AbstractStrategy<RemoteVehControlInput, RemoteVehControlOutput> {


    @Override
    public RemoteVehControlOutput call(RemoteVehControlInput input) {
        RemoteVehControlOutput output = this.vehAirConditionerSetTempProcess(input);
        return output;
    }

    @Override
    public RemoteVehControlOutput vehAirConditionerSetTempProcess(RemoteVehControlInput input) {
