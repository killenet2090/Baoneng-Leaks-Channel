...
@Slf4j
@Service
@ThirdPartySvcAnno(provider = Constant.PROVIDER_CODE, service = Constant.SMART_HOME_ACCOUNT_QRCODE)
public class SmartHomeAccountQrCodeServiceImpl implements SmartHomeAccountQrCodeIService, AbstractStrategy<AccountQrCodeInput, AccountQrCodeOutput> {


    @Override
    public AccountQrCodeOutput call(AccountQrCodeInput input) {
        AccountQrCodeOutput output = this.process(input);
        return output;
    }

    @Override
    public AccountQrCodeOutput process(AccountQrCodeInput input) {
        AccountQrCodeOutput output = new AccountQrCodeOutput();
        output.setVin(input.getVin());
        output.setQrcode("https://xxx.xxx.xxxx.png");
        output.setDeadline(120);
        return output;
    }
}
