...
public class JpushMsgDto {
     ...
     * 推送手机设备号
     */
    @ApiModelProperty(value = "推送手机设备号[和sendTagsList只能二选一 同时填只有一个生效]", name = "sendRegistrationIdList")
     ...
     * 发送标题
     */
    @ApiModelProperty(value = "发送标题", name = "sendTitle", example = "标题")
     ...
     */
    @ApiModelProperty(value = "发送标签列表[标签列表tags，一次推送最多 1000 个，和sendRegistrationId只能二选一 同时填只有一个生效]", name = "sendTagsList", example = "[001,002]")
    private List<String> sendTagsList;

    /**
     * 扩展数据
     */
    @ApiModelProperty(value = "扩展数据", name = "extraData", example = "{'bId':'T00002221222'}")
