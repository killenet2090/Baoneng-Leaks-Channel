...
@Component
@Slf4j
public class ChargingNotifyProducer {

    @Resource
    private KafkaTemplate<String, String> kafkaTemplate;

    @Value("${spring.kafka.producer.topics.charging-notify-topic}")
    private String notifyTopic;

    public void sendChargingNotify(MessageDto messageDto) {
        if (Objects.nonNull(messageDto.getVin())) {
            try {
                kafkaTemplate.send(notifyTopic, messageDto.getVin(), JsonUtil.toJson(messageDto));
            } catch (JsonProcessingException e) {
                log.error("发送充电通知到kafka发送异常[{}]", e.getMessage());
